function dev_token --argument subdomain username
  if test -n "$REPLACE_DEV_TOKEN"
    echo -n "$REPLACE_DEV_TOKEN"
    return 0
  end
  ...
  replace_token "$server" "$username" "$password" "$client_auth" "http"
end
